...
// Fallback function to convert HTML to clean text with SSML (for backward compatibility)
function processHtmlForTTS(html: string): string {
  if (!html) return '';
  
  // Simple HTML to text conversion with basic SSML
  let processed = html
...
serve(async (req) => {
  ...
  try {
    ...
    // Generate content hash from structured content if available, otherwise from HTML
    const contentForHash = page.content ? JSON.stringify(page.content) : (page.html_content || '')
    ...
    // Process content for TTS with emphasis and pauses
    // Prefer structured content over HTML for better accuracy
    let ttsText = '';
    if (page.content) {
      ttsText = processTipTapContentForTTS(page.content);
    } else {
      ttsText = processHtmlForTTS(page.html_content || '');
    }
